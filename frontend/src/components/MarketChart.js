...
// Найдём приближенную точку пересечения
function findIntersectionPoint(supplyData, demandData, price) {
  const supplyPoint = supplyData.find(p => p.price >= price) || supplyData[supplyData.length - 1];
  const demandPoint = demandData.find(p => p.price >= price) || demandData[demandData.length - 1];
  const avgVolume = (supplyPoint.volume + demandPoint.volume) / 2;
  return { price, volume: avgVolume };
}

const intersection = findIntersectionPoint(parsedSupply, parsedDemand, clearingPrice);
    ...
    <div className="chart-section">
      <ResponsiveContainer width="95%" height="90%">
        <LineChart>
          ...
          <XAxis dataKey="price" type="number" domain={[minPrice, maxPrice]} stroke="#ffffff" />
